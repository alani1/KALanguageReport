...
(function ($) {
      ...
      if ($YouTubeModal == null) {
        ...
        var modalContent = '<div class="modal-dialog" id="YouTubeModalDialog">' +
                              '<div class="modal-content" id="YouTubeModalContent">' +
                                ...
                                '<div id="YouTubeModalBody" style="padding:0;"></div>'+
                                '<div id="KALink">See Video & Exercises on Khan Academy</div>'+
                                '</div>' +
                              '</div>' +
                            '</div>';
  ...
  function setKAExercises(youtubeId) {
    var url = ["http://www.khanacademy.org/api/v1/videos/exercises", youtubeId].join('');
    $.ajax({
      url: url,
      dataType: 'jsonp',
      cache: true,
